...
func NewChainClient(providerType string, clientId string, clientSecret string, region string, networkId string, chainId string, providerUrl string, text string, UserKey string, UserCert string, SignKey string, SignCert string, ContractName string, ContractMethod string) (ChainClientInterface, error) {
	var res ChainClientInterface
	var err error
	if providerType == "ChainMaker" {
		res, err = newChainChainmakerClient(networkId, text, region, chainId, providerUrl, UserKey, UserCert, SignKey, SignCert, ContractName, ContractMethod)
		...
		res, err = newEthereumClient(providerUrl, clientSecret)
	} else {
