...
class VideoListPage extends BaseListPage {
  ...
  deleteVideo(record) {
    VideoBackend.deleteVideo(record)
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("video:Video deleted successfully"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("video:Video failed to delete")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("video:Video failed to delete")}: ${error}`);
      });
  }
  ...
  uploadFile(info) {
    ...
    if (status === "done") {
      if (res.status === "ok") {
        Setting.showMessage("success", i18next.t("video:Video uploaded successfully"));
        ...
        Setting.showMessage("error", `${i18next.t("video:Video failed to upload")}: ${res.msg}`);
      }
    } else if (status === "error") {
      Setting.showMessage("success", i18next.t("video:Video failed to upload"));
    }
  }
