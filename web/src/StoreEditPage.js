...
class StoreEditPage extends React.Component {
  ...
  getStorageSubpathLabel(storageProvider) {
    ...
    if (providerSelected && providerSelected.type === "OpenAI File System") {
      return Setting.getLabel(i18next.t("provider:Mirror Storage subpath"), i18next.t("provider:Mirror Storage subpath - Tooltip"));
    }
    return Setting.getLabel(i18next.t("provider:Storage subpath"), i18next.t("provider:Storage subpath - Tooltip"));
  }
  ...
  renderStore() {
    return (
      <Card size="small" title={
        ...
        <Row style={{marginTop: "20px"}} >
          <Col style={{marginTop: "5px"}} span={(Setting.isMobile()) ? 22 : 2}>
            {this.getStorageSubpathLabel(this.state.store.storageProvider)} :
          </Col>
          <Col span={22} >
