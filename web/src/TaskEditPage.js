    ...
    TaskBackend.getTask(this.props.account.name, this.state.taskName)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("task:Failed to get task")}: ${res.msg}`);
        }
      });
  }
    ...
    MessageBackend.getAnswer(provider, question, framework, video)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("task:Failed to get answer")}: ${res.msg}`);
        }
    ...
    ProviderBackend.getProviders(this.props.account.name)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("task:Failed to get providers")}: ${res.msg}`);
        }
      });
  }
  ...
  submitTaskEdit(exitAfterSave) {
    ...
    TaskBackend.updateTask(this.state.task.owner, this.state.taskName, task)
      .then((res) => {
        if (res.status === "ok") {
          if (res.data) {
            Setting.showMessage("success", i18next.t("task:Successfully saved"));
            this.setState({
            ...
            Setting.showMessage("error", i18next.t("task:failed to save: server side failure"));
            this.updateTaskField("name", this.state.taskName);
          }
        } else {
          Setting.showMessage("error", `${i18next.t("task:failed to save")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("task:failed to save")}: ${error}`);
      });
  }
