...
class BrowserSpeechToTextProvider {
  ...
  initBrowserRecognition(resultCallback) {
    ...
    if (!SpeechRecognition) {
      Setting.showMessage("error", i18next.t("provider:Speech recognition is not supported in this browser."));
      return null;
    }
    ...
    this.recognition.onerror = (event) => {
      if (event.error !== "aborted") {
        Setting.showMessage("error", `${i18next.t("provider:Speech recognition error")}: ${event.error}`);
      }
    };
    ...
    try {
      ...
      Setting.showMessage("error", `${i18next.t("provider:Failed to start speech recognition")}: ${error.message}`);
      this.recognition = null;
      return null;
    }
  }
