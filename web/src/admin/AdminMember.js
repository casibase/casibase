...
class AdminMember extends React.Component {
  ...
  getMember() {
    ...
    MemberBackend.getMemberAdmin(this.state.memberId)
      .then((res) => {
        this.setState({
          member: res,
        }, () => {
  ...
  updateMemberInfo() {
    MemberBackend.updateMember(this.state.memberId, this.state.form)
      .then((res) => {
        if (res.status === 'ok') {
          this.getMember();
          this.setState({
  ...
  getSearchResult() {
    ...
    MemberBackend.getMembersAdmin(this.state.cs, this.state.us, this.state.un, this.state.limit, 1)
      .then((res) => {
        if (res.status === 'ok') {
  ...
  showPageColumn() {
    ...
    return (
      <PageColumn page={this.state.page} total={this.state.membersNum} url={this.state.url} defaultPageNum={this.state.limit} />
    );
  }
  ...
  renderRadioButton(type) {
    switch (type) {
      case "us":
        return (
          <span>
            <input type="radio" onClick={() => this.setState({us: 1})} checked={this.state.us === 1} name="us" />{i18next.t("member:Asc")}{" "}
            <input type="radio" onClick={() => this.setState({us: 2})} checked={this.state.us === 2} name="us" />{i18next.t("member:Desc")}{" "}
            <input type="radio" onClick={() => this.setState({us: 0})} checked={this.state.us === 0} name="us" />{i18next.t("member:Ignore")}
          </span>
        );
      case "cs":
        return (
          <span>
            <input type="radio" onClick={() => this.setState({cs: 1})} checked={this.state.cs === 1} name="cs" />{i18next.t("member:Asc")}{" "}
            <input type="radio" onClick={() => this.setState({cs: 2})} checked={this.state.cs === 2} name="cs" />{i18next.t("member:Desc")}{" "}
            <input type="radio" onClick={() => this.setState({cs: 0})} checked={this.state.cs === 0} name="cs" />{i18next.t("member:Ignore")}
          </span>
        );
    }
  }
      ...
      <table cellPadding="5" cellSpacing="0" border="0" width="100%">
        ...
        <tr>
          ...
          <td width="auto" align="left">
            <input type="submit" className="super normal button" value={i18next.t("member:Search")} onClick={() => this.getSearchResult()}/>
          </td>
        </tr>
        </tbody>
      </table>
  ...
  renderManagementList(item){
    return (
      <a href="javascript:void(0);" className={this.state.event === item.value ? "tab_current" : "tab"} onClick={() => this.changeEvent(item.value)}>{i18next.t(`member:${item.label}`)}</a>
    );
  }
  ...
  renderMemberStatus(status) {
    switch (status) {
      ...
      default:
        return (
          <span className="gray">
            {i18next.t("member:Unknown status")}
          </span>
        );
    }
  }
  ...
  render() {
    if (this.state.memberId !== undefined) {
      ...
      if (this.state.event === "basic") {
        let balance = member?.goldCount * 10000 + member?.silverCount * 100 + member?.bronzeCount;
            ...
            <div className="box">
              ...
              <div className="inner">
                <table cellPadding="5" cellSpacing="0" border="0" width="100%">
                  <tbody>
                  <tr>
                    <td width="120" align="right">{i18next.t("member:Username")}</td>
                    <td width="auto" align="left">
                      <span className="gray">
                        {member?.id}
                      </span>
                  ...
                  <tr>
                    <td width="120" align="right">{i18next.t("member:Topic num")}</td>
                    <td width="auto" align="left">
                      <span className="gray">
                        {member?.topicNum}
                      </span>
                  ...
                  <tr>
                    <td width="120" align="right">{i18next.t("member:Reply num")}</td>
                    <td width="auto" align="left">
                      <span className="gray">
                        {member?.replyNum}
                      </span>
                  ...
                  <tr>
                    <td width="120" align="right">{i18next.t("member:Balance")}</td>
                    <td width="auto" align="left">
                      <span className="gray">
                        {balance}
                      </span>
                  ...
                  <tr>
                    <td width="120" align="right">{i18next.t("member:Latest login")}</td>
                    <td width="auto" align="left">
                      <span className="gray">
                        {member?.latestLogin}
                      </span>
                  ...
                  <tr>
                    <td width="120" align="right">{i18next.t("member:Files num")}</td>
                    <td width="auto" align="left">
                      <span className="gray">
                  ...
                  <tr>
                    <td width="120" align="right">{i18next.t("member:File quota")}</td>
                    <td width="auto" align="left">
                      <input type="range" min="0" max="1000" step="1"
                             value={this.state.form?.fileQuota === undefined ? 0 : this.state.form?.fileQuota}
                             onChange={event => this.updateFormField("fileQuota", parseInt(event.target.value))}/>
                      &nbsp;{" "}&nbsp;{" "}
                      <input type="number" name="sorter" min="0" max="1000" step="1" value={this.state.form?.fileQuota}
                             style={{width: "50px"}}
                             onChange={event => this.updateFormField("fileQuota", parseInt(event.target.value))}/>
                    </td>
                  </tr>
                  <tr>
                    <td width="120" align="right">{i18next.t("member:Status")}</td>
                    <td width="auto" align="left">
                      <Select2
                        value={this.getIndexFromStatus(this.state.form?.status)}
                        style={{width: "300px", fontSize: "14px"}}
                        data={
                          this.state.Status_LIST.map((status, i) => {
                            return {text: `${i18next.t(`member:${status.label}`)}`, id: i};
                          })
                        }
                        onSelect={event => {
                        ...
                        options={
                          {
                            placeholder: i18next.t("member:Please choose member's status"),
                          }
                        }
                      />
                    </td>
                  ...
                  <tr>
                    ...
                    <td width="auto" align="left">
                      <input type="submit" className="super normal button" value={i18next.t("member:Save")} onClick={() => this.updateMemberInfo()}/>
                    </td>
                  </tr>
                  </tbody>
                </table>
              ...
              <table cellPadding="5" cellSpacing="0" border="0" width="100%">
                ...
                <tr>
                  <td width="120" align="right">
                    <Avatar username={member?.id} size="small" avatar={member?.avatar}/>
                  </td>
                  <td width="auto" align="left">
                    {Setting.getForumName()} {i18next.t("member:No.")} {member?.no} {i18next.t("member:member")}
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Phone")}</td>
                ...
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Email")}</td>
                  <td width="200" align="left">
                    {
                ...
                <tr>
                  <td width="120" align="right">Google</td>
                  <td width="200" align="left">
                    {
                ...
                <tr>
                  <td width="120" align="right">Github</td>
                  <td width="200" align="left">
                    {
                ...
                <tr>
                  <td width="120" align="right">{i18next.t("setting:WeChat")}</td>
                  <td width="200" align="left">
                    {
                ...
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Website")}</td>
                  <td width="200" align="left">
                    {member?.website}
                  </td>
                </tr>
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Company")}</td>
                  <td width="200" align="left">
                    {member?.company}
                  </td>
                </tr>
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Company title")}</td>
                  <td width="200" align="left">
                    {member?.companyTitle}
                  </td>
                </tr>
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Location")}</td>
                  <td width="200" align="left">
                    {member?.location}
                  </td>
                </tr>
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Tagline")}</td>
                  <td width="200" align="left">
                    {member?.tagline}
                  </td>
                </tr>
                <tr>
                  <td width="120" align="right">{i18next.t("setting:Bio")}</td>
                  <td width="200" align="left">
                    {member?.bio}
                  </td>
                </tr>
    ...
    return (
      <div className="box">
        ...
        <div className="cell">
          ...
          >
            <Panel header={<span style={{color: "#666"}}>{i18next.t("member:Condition search")}</span>} >
              {
