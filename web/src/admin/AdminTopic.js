...
class AdminTopic extends React.Component {
    ...
    showPageColumn() {
        ...
        return <PageColumn page={this.state.page} total={this.state.topicsNum} url={this.state.url}
    ...
    renderRadioButton(type) {
        switch (type) {
            ...
            <input type="radio" onClick={() => this.setState({sdt: 1})} checked={this.state.sdt === 1} name="sdt"/>
                        {i18next.t("admin:Show")} <input type="radio" onClick={() => this.setState({sdt: 0})}
                                                         checked={this.state.sdt === 0} name="sdt"/>
            ...
            <input type="radio" onClick={() => this.setState({us: 1})} checked={this.state.us === 1} name="us"/>
                        {i18next.t("admin:Asc")} <input type="radio" onClick={() => this.setState({us: 2})}
                                                        checked={this.state.us === 2} name="us"/>
                        {i18next.t("admin:Desc")} <input type="radio" onClick={() => this.setState({us: 0})}
                                                         checked={this.state.us === 0} name="us"/>
            ...
            <input type="radio" onClick={() => this.setState({cs: 1})} checked={this.state.cs === 1} name="cs"/>
                        {i18next.t("admin:Asc")} <input type="radio" onClick={() => this.setState({cs: 2})}
                                                        checked={this.state.cs === 2} name="cs"/>
                        {i18next.t("admin:Desc")} <input type="radio" onClick={() => this.setState({cs: 0})}
                                                         checked={this.state.cs === 0} name="cs"/>
                        {i18next.t("admin:Ignore")}
          </span>
                );
            case "lrs":
            ...
            <input type="radio" onClick={() => this.setState({lrs: 1})} checked={this.state.lrs === 1} name="lrs"/>
                        {i18next.t("admin:Asc")} <input type="radio" onClick={() => this.setState({lrs: 2})}
                                                        checked={this.state.lrs === 2} name="lrs"/>
                        {i18next.t("admin:Desc")} <input type="radio" onClick={() => this.setState({lrs: 0})}
                                                         checked={this.state.lrs === 0} name="lrs"/>
                        {i18next.t("admin:Ignore")}
          </span>
                );
            case "rcs":
            ...
            <input type="radio" onClick={() => this.setState({rcs: 1})} checked={this.state.rcs === 1} name="rcs"/>
                        {i18next.t("admin:Asc")} <input type="radio" onClick={() => this.setState({rcs: 2})}
                                                        checked={this.state.rcs === 2} name="rcs"/>
                        {i18next.t("admin:Desc")} <input type="radio" onClick={() => this.setState({rcs: 0})}
                                                         checked={this.state.rcs === 0} name="rcs"/>
                        {i18next.t("admin:Ignore")}
          </span>
                );
            case "hs":
            ...
            <input type="radio" onClick={() => this.setState({hs: 1})} checked={this.state.hs === 1} name="hs"/>
                        {i18next.t("admin:Asc")} <input type="radio" onClick={() => this.setState({hs: 2})}
                                                        checked={this.state.hs === 2} name="hs"/>
                        {i18next.t("admin:Desc")} <input type="radio" onClick={() => this.setState({hs: 0})}
                                                         checked={this.state.hs === 0} name="hs"/>
                        {i18next.t("admin:Ignore")}
          </span>
                );
            case "fcs":
            ...
            <input type="radio" onClick={() => this.setState({fcs: 1})} checked={this.state.fcs === 1} name="fcs"/>
                        {i18next.t("admin:Asc")} <input type="radio" onClick={() => this.setState({fcs: 2})}
                                                        checked={this.state.fcs === 2} name="fcs"/>
                        {i18next.t("admin:Desc")} <input type="radio" onClick={() => this.setState({fcs: 0})}
                                                         checked={this.state.fcs === 0} name="fcs"/>
                        {i18next.t("admin:Ignore")}
          </span>
                );
            default:
            ...
            <table cellPadding="5" cellSpacing="0" border="0" width="100%">
                ...
                <tr>
                    ...
                    <td width="auto" align="left">
                        <input type="submit" className="super normal button" value={i18next.t("topic:Search")}
    ...
    renderManagementList(item) {
        return (
            <a href="javascript:void(0);" className={this.state.event === item.value ? "tab_current" : "tab"}
        ...
        return (
            <div className="box">
                <div className="header">
                    ...
                    <Link to="/admin">{i18next.t("admin:Backstage management")}</Link> <span
                    className="chevron">&nbsp;›&nbsp;</span>
                    <Link to="/admin/topic">{i18next.t("topic:Topic management")}</Link> <span
                    className="chevron">&nbsp;›&nbsp;</span>
                    ...
                    <div class="box">
                        <div className="box">
                            <div className="header">
                                ...
                                className="chevron">&nbsp;›&nbsp;</span> {i18next.t("error:Topic does not exist")}
                            </div>
                            <div className="cell">
                        ...
                        <span className="gray">
                          <Link to={`/go/${encodeURIComponent(topic?.nodeId)}`} target="_blank">
                            {topic?.nodeName}
                                            ...
                                            &nbsp; &nbsp;{" "}
                                            <Link to={`/move/topic/${topic?.id}`} style={{fontWeight: "bolder"}}
                                                  target="_blank">
                                    ...
                                    <tr>
                                        ...
                                        <td width="auto" align="left">
                                            <span
                                                className="gray">{topic.lastReplyUser === "" ? i18next.t("reply:No reply yet") :
                                                <Link
                                                    to={`/member/${topic?.lastReplyUser}`}>{topic?.lastReplyUser}</Link>}</span>
                                        </td>
                                    </tr>
                                    {topic.lastReplyUser !== "" ? (
                                        <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td width="auto" align="left">
                                            <span
                                                className="gray">{this.renderTopicStatus(topic?.deleted, topic?.homePageTopTime, topic?.tabTopTime, topic?.nodeTopTime)}</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td width="auto" align="left">
                                            <span
                                                className="gray">{i18next.t("topic:Please change the basic information through the topic page")}</span>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
                            ...
                            <table cellPadding="5" cellSpacing="0" border="0" width="100%">
                                ...
                                <tr>
                                    ...
                                    <td width="500" align="left">
                                        <Link to={`/t/${topic?.id}?from=${encodeURIComponent(window.location.href)}`}
                                              target="_blank">
                                            {topic?.title}
                                        </Link>
                                    </td>
        ...
        return (
            <div className="box">
                ...
                <div className="cell">
                    ...
                    >
                        <Panel header={<span
                            style={{color: "#666"}}>{i18next.t("admin:Condition search")}</span>}>{this.renderSearchList()}</Panel>
                    </Collapse>
                </div>
                {this.showPageColumn()}
                <div id="all-tabs">
