  ...
  getMenuItems() {
    ...
    if (Conf.ShortcutPageItems.length > 0 && domain === "data") {
      ...
      res.push(Setting.getItem(<Link style={{color: textColor}} to="/chat">{i18next.t("general:Home")}</Link>, "/home", <HomeTwoTone twoToneColor={twoToneColor} />, [
        ...
        Setting.getItem(<Link to="/desktop">{i18next.t("general:Web Desktop")}</Link>, "/desktop"),
      ]));
  ...
  renderRouter() {
    ...
    return (
      <Switch>
        ...
        <Route path="/desktop" component={(props) => <WebDesktop account={this.state.account} {...props} />} />
