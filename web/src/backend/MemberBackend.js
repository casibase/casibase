...
export function updateMemberAvatar(avatar) {
  return fetch(`${Setting.ServerUrl}/api/update-member-avatar?avatar=${avatar}`, {
    method: "POST",
    credentials: "include"
  }).then(res => res.json());
}
...
export function updateMemberEmailReminder(status) {
  return fetch(`${Setting.ServerUrl}/api/update-member-email-reminder?status=${status}`, {
    method: "POST",
    credentials: "include"
  }).then(res => res.json());
}
...
export function googleLogin(code, state, redirectUrl, addition) {
  return fetch(`${Setting.ServerUrl}/api/auth/google?code=${code}&state=${state}&redirect_url=${redirectUrl}&addition=${addition}`, {
    method: 'GET',
    credentials: 'include',
  }).then(res => res.json());
}
...
export function qqLogin(code, state, redirectUrl, addition) {
  console.log(redirectUrl)
  return fetch(`${Setting.ServerUrl}/api/auth/qq?code=${code}&state=${state}&redirect_url=${redirectUrl}&addition=${addition}`, {
    method: 'GET',
    credentials: 'include',
  }).then(res => res.json());
}
