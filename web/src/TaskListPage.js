...
class TaskListPage extends BaseListPage {
  ...
  addTask() {
    ...
    TaskBackend.addTask(newTask)
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("task:Successfully added"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("task:Failed to add")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("task:Failed to add")}: ${error}`);
      });
  }
  ...
  deleteTask(record) {
    TaskBackend.deleteTask(record)
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("task:Successfully deleted"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("task:Failed to delete")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("task:Failed to delete")}: ${error}`);
      });
  }
