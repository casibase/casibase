    ...
    UsageBackend.getUsers(serverUrl, this.props.account.name)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("general:Failed to get users")}: ${res.msg}`);
        }
      });
  }
    ...
    UsageBackend.getUserTableInfos(serverUrl, this.props.account.name)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("general:Failed to get userTableInfo")}: ${res.msg}`);
        }
      });
  }
