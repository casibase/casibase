    ...
    VideoBackend.getVideo(this.state.owner, this.state.videoName)
      .then((res) => {
        ...
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("video:Failed to get video")}: ${res.msg}`);
        }
      });
  }
    ...
    TaskBackend.getTasks(this.props.account.name)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("video:Failed to get tasks")}: ${res.msg}`);
        }
      });
  }
  ...
  submitVideoEdit(exitAfterSave) {
    ...
    VideoBackend.updateVideo(this.state.video.owner, this.state.videoName, video)
      .then((res) => {
        if (res.status === "ok") {
          if (res.data) {
            Setting.showMessage("success", i18next.t("video:Successfully saved"));
            this.setState({
            ...
            Setting.showMessage("error", i18next.t("video:failed to save: server side failure"));
            this.updateVideoField("name", this.state.videoName);
          }
        } else {
          Setting.showMessage("error", `${i18next.t("video:failed to save")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("video:failed to save")}: ${error}`);
      });
  }
