...
class VectorListPage extends BaseListPage {
  ...
  addVector() {
    ...
    VectorBackend.addVector(newVector)
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("vector:Vector added successfully"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("vector:Failed to add vector")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("vector:Vector failed to add")}: ${error}`);
      });
  }
  ...
  deleteVector(record) {
    VectorBackend.deleteVector(record)
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("vector:Vector deleted successfully"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("vector:Vector failed to delete")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("vector:Vector failed to delete")}: ${error}`);
      });
  }
  ...
  deleteAllVectors() {
    VectorBackend.deleteAllVectors()
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("vector:All vectors deleted successfully"));
          this.setState({
            data: [],
            pagination: {
              ...this.state.pagination,
          ...
          Setting.showMessage("error", `${i18next.t("vector:Vectors failed to delete")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("vector:Vectors failed to delete")}: ${error}`);
      });
  }
