                ...
                <div class="fr" align="right">
                    ...
                    {this.props.account !== undefined && this.props.account !== null && this.props.account?.name !== this.state.topic?.author ? (
                        this.state.topic?.thanksStatus === false ? (
                            <div id="topic_thank">
                                <a href="#;"
                                   onClick={() => this.thanksTopic(this.state.topic?.id, this.state.topic?.author)}
                                   className="op">
    ...
    render() {
            ...
            return (
                <div className="box">
                    <div className="header">
                        {Setting.getHomeLink()} <span
                        className="chevron">&nbsp;›&nbsp;</span> {i18next.t("loading:Topic is loading")}
                    </div>
                    <div className="cell">
        ...
        if (this.state.topic === null) {
            return (
                <div class="box">
                    <div class="header">
                        {Setting.getHomeLink()} <span
                        class="chevron">&nbsp;›&nbsp;</span> {i18next.t("error:Topic not found")}
                    </div>
                    <div class="cell">
        ...
        if (this.state.event === "review") {
            ...
            return (
                <div class="box">
                    <div class="header">
                        {Setting.getHomeLink()} <span class="chevron">&nbsp;›&nbsp;</span> <Link
                        to={`/go/${encodeURIComponent(this.state.topic?.nodeId)}`}>{this.state.topic?.nodeName}</Link>
                        <span class="chevron">&nbsp;›&nbsp;</span>{" "}
                        <Link to={`/t/${this.state.topic?.id}`}>{pangu.spacing(this.state.topic?.title)}</Link> <span
                        class="chevron">&nbsp;›&nbsp;</span> Review
                    </div>
                    <div class="cell topic_content markdown_body">
                        <p>
                            {i18next.t("topic:The new topic has been successfully created on the")} <Link
                            to={`/go/${encodeURIComponent(this.state.topic?.nodeId)}`}>{this.state.topic?.nodeName}</Link>{" "}
                        ...
                        <ul>
                            <li>
                                ...
                                <Link
                                    to={`/go/${encodeURIComponent(this.state.topic?.nodeId)}`}>{this.state.topic?.nodeName}</Link> {i18next.t("topic:node, within 10 minutes after creation, you can")}{" "}
            ...
            <div>
                <div className={`box ${this.state.topic.nodeId}`} style={{borderBottom: "0px"}}>
                    <div className={`header ${this.state.topic.nodeId}`}>
                        <div className="fr">
                            <Avatar username={this.state.topic?.author} size={pcBrowser ? "large" : "middle"}
                  ...
                  <a href="#;" onClick={this.upVoteTopic()} className={`vote ${this.state.topic.nodeId}`}>
                    ...
                    <li className="fa fa-chevron-down"/>
                        ...
                        <small className="gray">
                            <UserLink username={this.state.topic?.author}
                                      classNameText={`${this.state.topic.nodeId}`}/> · {Setting.getPrettyDate(this.state.topic?.createdTime)} · {this.state.topic?.hitCount} {i18next.t("topic:hits")}
                            &nbsp;{" "}
                            {this.props.account?.isAdmin ? (
                                <span>
                    ...
                    <div className={`cell ${this.state.topic.nodeId}`}>
                        <div className={`topic_content ${this.state.topic.nodeId}`}>
                          ...
                          {`Translate from ${this.state.translation.from} by  `}
                                                    <img height={18}
                                                         src="https://cdn.casbin.org/img/logo_google.svg"></img>
                        </span>
                                            ) : (
                                                <span>Translate</span>
                                            )}
