...
class MemberBox extends React.Component {
            ...
            return (
                <div className="box">
                    <div className="header">
                        ...
                        className="chevron">&nbsp;›&nbsp;</span> {i18next.t("loading:Member profile is loading")}
                    </div>
                    <div className="cell">
        ...
        if (this.state.member === null) {
            return (
                <div className="box">
                    <div className="header">
                        <Link to="/">{Setting.getForumName()}</Link> <span
                        className="chevron">&nbsp;›&nbsp;</span> {i18next.t("error:Member does not exist")}
                    </div>
                    <div className="cell">
            ...
            <div className="box">
                <div className="cell">
                    <table cellPadding="0" cellSpacing="0" border="0" width="100%">
                        ...
                        <tr>
                            <td width={Setting.PcBrowser ? "73" : "56"} valign="top" align="center">
                                <Avatar username={this.state.member?.name} size={Setting.PcBrowser ? "large" : "middle"}
                                        avatar={this.state.member?.avatar}/>
                                <div className="sep10"/>
                                {this.state.member?.onlineStatus ?
                                    <strong className="online"> {i18next.t("member:ONLINE")} </strong> : ""}
                            </td>
                            <td width="10"/>
                            <td width="auto" valign="top" align="left">
                                <div className="fr" style={{display: showWatch ? "" : "none"}}>
                                    {this.state.favoritesStatus ? (
                                        <input type="button" value={i18next.t("member:Cancel Following")}
                                               onClick={() => this.deleteFavorite(this.state.member?.name)}
                                               className="super inverse button"/>
                                    ) : (
                                        <input type="button" value={i18next.t("member:Watch")}
                                               onClick={() => this.addFavorite(this.state.member?.name)}
                                               className="super special button"/>
                                    )}
                                    <div className="sep10"/>
                                    <input type="button" value="Block" onClick={this.block(this.state.memberId)}
                    ...
                    {Setting.getForumName()} {i18next.t("member:No.")} {this.state.member?.ranking} {i18next.t("member:member, joined on")} {Setting.getFormattedDate(this.state.member?.createdTime)}
                                    ...
                                    {this.state.member?.isModerator ?
                                        <img src={Setting.getStatic("/img/mod@2x.png")} height="14px"
                                             align="absmiddle"/> : null} {this.state.member?.isModerator ? i18next.t("member:authorized to manage the community") : null}
                  </span>
                                <div className="sep10"/>
                                <div className="balance_area">
                                    {goldCount} <img src={Setting.getStatic("/img/gold@2x.png")} height="16" alt="G"
                                                     border="0"/> {silverCount} <img
                                    src={Setting.getStatic("/img/silver@2x.png")} height="16" alt="S"
                                    border="0"/> {bronzeCount}{" "}
                ...
                <div className="widgets">
                    {this.state.member?.homepage?.length !== 0 ? (
                        <a href={this.state.member?.homepage} className="social_label" target="_blank"
                           rel="nofollow noopener noreferrer">
                            <img src={Setting.getStatic("/img/social_home.png")} width="24" alt="Website"
                                 align="absmiddle"/> &nbsp;{this.state.member?.homepage}
                        </a>
                    ) : null}
                    {this.state.member?.location?.length !== 0 ? (
                        <a href={`http://www.google.com/maps?q=${this.state.member?.location}`} className="social_label"
                           target="_blank" rel="nofollow noopener noreferrer">
                            <img src={Setting.getStatic("/img/social_geo.png")} width="24" alt="Geo"
                                 align="absmiddle"/> &nbsp;{this.state.member?.location}
                        </a>
                    ) : null}
    ...
    renderIdp(user, providerType) {
        ...
        return (
            <a href={profileUrl} style={profileUrl !== "" ? null : {pointerEvents: "none"}} className="social_label"
               target="_blank" rel="nofollow noopener noreferrer">
                <img src={avatarUrl} width="24" style={{borderRadius: "24px"}} alt={providerType}
                     align="absmiddle"/> &nbsp;{name}
            </a>
        );
    }
