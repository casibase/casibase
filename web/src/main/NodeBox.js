...
class NodeBox extends React.Component {
    ...
    showPageColumn() {
        ...
        return <PageColumn page={this.state.page} total={this.state.topicNum} url={this.state.url}
                           nodeId={this.state.nodeId} defaultPageNum={this.state.limit}/>;
    }
        ...
        return (
            <div className="header">
                <div className="fr f12">
                    <span className="snow">{i18next.t("node:Total topics")}</span> <strong
                    className="gray">{this.state.topicNum}</strong> {this.props.account !== null ?
                    <span className="snow">&nbsp;•&nbsp;</span> : null}{" "}
                ...
                className="chevron">&nbsp;›&nbsp;</span> {this.state.nodeInfo?.name}
                <div className="sep5"></div>
                {this.props.account !== null ? (
                    <div align="right">
                        <input type="button" className="super normal button" value={i18next.t("node:new topic")}
                               onClick={() => this.props.history.push(`/new/${this.state.nodeId}`)}
        ...
        return (
            ...
            <span className="gray">
              {this.state.nodeInfo?.moderators !== null && this.state.nodeInfo?.moderators.length !== 0 ? (
                  <span>
                  {this.props.account?.isAdmin ? <Link
                          to={`/go/${encodeURIComponent(this.state.nodeId)}/moderators`}>{i18next.t("node:Moderator")}</Link> :
                  ...
                  <Link
                      to={this.props.account?.isAdmin ? `/go/${encodeURIComponent(this.state.nodeId)}/moderators` : null}>{i18next.t("node:No moderators")}</Link>
              ) : (
                ...
                &nbsp;
                        ...
                        <div>{this.state.nodeInfo.mailingList === "" ? i18next.t("node:No mailing list yet") :
                            <a href={"https://groups.google.com/g/" + this.state.nodeInfo.mailingList.split("@")[0]}>{this.state.nodeInfo.mailingList}</a>}</div>
                    </div>
            ...
            />
                    ...
                    <div className="node_header_tabs">
                        <Link to={`/go/${encodeURIComponent(nodeId)}`}
                              className={!this.props.match.params.event ? "node_header_tab_current" : "node_header_tab"}>
                            {i18next.t("node:All topics")}
                        </Link>
                        <Link to={`/go/${encodeURIComponent(nodeId)}/links`}
                              className={this.props.match.params.event ? "node_header_tab_current" : "node_header_tab"}>
        ...
        return (
            <div className={`box ${this.state.nodeId}`}>
                ...
                <div className="cell" align="center">
                    <div
                        className="fr">{`${this.state.favoritesNum} ${i18next.t("node:members have added this node to favorites")}`}</div>
                    <span
                        className="gray">{`${i18next.t("node:Topic")} ${from} ${i18next.t("node:to")} ${end} ${i18next.t("node:of all")} ${this.state.topicNum} ${i18next.t("node:topics")}`}</span>
                </div>
            </div>
        );
    }
        ...
        if (this.state.nodeInfo === null) {
            return (
                <div id={pcBrowser ? "Main" : ""}>
                  ...
                  <br/>
                  <span className="chevron">‹</span> &nbsp;
                                    {i18next.t("error:Back to")} <Link
                                    to={`/member/${this.props.account?.name}`}>{i18next.t("error:My profile")}</Link>
                </span>
                            )}
                        </div>
                    </div>
                        ...
                        <table cellPadding="5" cellSpacing="0" border="0" width="100%">
                            ...
                            {this.state.nodeInfo?.moderators !== null && this.state.nodeInfo?.moderators.length !== 0 ? (
                                <tr>
                                    ...
                                    <td width="auto" align="left">
                                        <Link to={`/member/${this.state.nodeInfo?.moderators[0]}`}
                                              style={{fontWeight: "bolder"}} target="_blank">
                    ...
                    <div className="cell">
                        <table cellPadding="5" cellSpacing="0" border="0" width="100%">
                            ...
                            <tr>
                                ...
                                <td width="auto" align="left">
                                    <input type="text" className="sl" name="title" maxLength="64"
                                           value={this.state.newModerator}
                                           onChange={(event) => this.inputNewModerator(event.target.value)}/>
                                </td>
                            </tr>
                            <tr>
                                <td width="120" align="right"></td>
                                <td width="auto" align="left">
                                    <input type="submit" className="super normal button" value={i18next.t("node:Add")}
                                           onClick={() => this.addNodeModerator(this.state.newModerator)}/>
                                </td>
                            </tr>
                            </tbody>
                        </table>
        ...
        return (
            <div id={pcBrowser ? "Main" : ""}>
                ...
                {this.props.account !== undefined && this.props.account !== null && pcBrowser ?
                    <NewNodeTopicBox nodeId={this.state.nodeId} account={this.props.account} size={"small"}
                                     refreshAccount={this.props.refreshAccount.bind(this)}/> : null}
            </div>
        );
    }
}
