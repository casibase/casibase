...
class LatestReplyBox extends React.Component {
    ...
    showPageColumn() {
        ...
        return <PageColumn page={this.state.page} total={this.state.repliesNum} url={this.state.url}
                    ...
                    <table cellPadding="0" cellSpacing="0" border="0" width="100%">
                        ...
                        <tr>
                            ...
                            >
                                ...
                                <span className="gray">
                    {i18next.t("member:replied")} <Link
                                    to={`/member/${reply.topicAuthor}`}> {reply.topicAuthor} </Link> {i18next.t("member:'s topic")}
                                    <span className="chevron">›</span> <Link
                                    to={`/go/${reply.nodeId}`}> {reply.nodeName} </Link>
                    <span className="chevron">›</span> <Link
                                    to={`/t/${reply.topicId}?from=${encodeURIComponent(window.location.href)}`}> {pangu.spacing(reply.topicTitle)} </Link>
                  </span>
                            </td>
                        </tr>
                        </tbody>
    ...
    render() {
            ...
            return (
                <div className="box">
                    <div className="header">
                        ...
                        <Link to={`/member/${this.state.memberId}`}> {this.state.memberId}</Link> <span
                        className="chevron">&nbsp;›&nbsp;</span> {i18next.t("member:All Replies")}
                        <div className="fr f12">
                            <span className="snow">{i18next.t("member:Total Replies")}&nbsp;</span> <strong
                            className="gray">{this.state.repliesNum}</strong>
                        </div>
                    </div>
        ...
        return (
            <div className="box">
                ...
                <div className="inner">
                    <span className="chevron">»</span> <Link
                    to={`/member/${this.state.memberId}/replies`}>{`${this.state.memberId}${i18next.t("member:'s more replies")}`}</Link>
                </div>
            </div>
        );
    }
