  ...
  renderNodes(node) {
    return (
      <div class="node rightFavorite node-casbin">
        ...
        &nbsp;{" "}
        <Link to={`/go/${node?.id}`} id="linkTitle">{node?.name}</Link>
      </div>
    );
  }
      ...
      <div class={`box ${this.state.nodeId}`}>
            ...
            <span>
              ...
              <div className="nodes-sidebar-container">
                ...
                }
                {
                  relatedNum > 5 ?
                    <Collapse
                      onChange={() => this.changeShowRelatedNode()}
                    >
                      <Panel
                        header={<span><i className={this.state.showRelatedNode ? "fa fa-caret-down" : "fa fa-caret-right"}> &nbsp;
                          <span>{this.state.showRelatedNode ? `${i18next.t("bar:Collapse the extra nodes below")}` : `${i18next.t("bar:show")} ${relatedNum - 5} ${i18next.t("bar:more related nodes")}`}</span></i></span>}
                        showArrow={false} className="toggle-more-nodes">
                        {
        ...
        }
        {
          childNum !== 0 ?
            <span>
              <div className="cell-top">
                <span className="f12 gray"><div className="fr">{childNum}</div>
                  {i18next.t("bar:Child nodes")}</span>
              </div>
              <div className="nodes-sidebar-container">
                {
                ...
                }
                {
                  childNum > 5 ?
                    <Collapse
                      onChange={() => this.changeShowChildNode()}
                    >
                      <Panel header={<span><i className={this.state.showChildNode ? "fa fa-caret-down" : "fa fa-caret-right"}> &nbsp;<span>{this.state.showChildNode ? `${i18next.t("bar:Collapse the extra nodes below")}` : `${i18next.t("bar:show")} ${childNum - 5} ${i18next.t("bar:more child nodes")}`}</span></i></span>} showArrow={ false } className="toggle-more-nodes">
                        {
