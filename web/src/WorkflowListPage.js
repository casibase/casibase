...
class WorkflowListPage extends BaseListPage {
  ...
  addWorkflow() {
    ...
    WorkflowBackend.addWorkflow(newWorkflow)
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("workflow:Workflow added successfully"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("workflow:Failed to add workflow")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("workflow:Workflow failed to add")}: ${error}`);
      });
  }
  ...
  deleteWorkflow(record) {
    WorkflowBackend.deleteWorkflow(record)
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("workflow:Workflow deleted successfully"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("workflow:Workflow failed to delete")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("workflow:Workflow failed to delete")}: ${error}`);
      });
  }
