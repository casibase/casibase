...
class ImageListPage extends BaseListPage {
    ...
    ImageBackend.addImage(newImage)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("success", i18next.t("image:Successfully added"));
        } else {
          Setting.showMessage("error", `${i18next.t("image:Failed to add")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("image:Failed to add")}: ${error}`);
      });
  }
  ...
  deleteImage(i) {
    ImageBackend.deleteImage(this.state.data[i])
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", i18next.t("image:Successfully deleted"));
          this.setState({
          ...
          Setting.showMessage("error", `${i18next.t("image:Failed to delete")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("image:Failed to delete")}: ${error}`);
      });
  }
    ...
    MachineBackend.addMachine(newMachine)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("success", i18next.t("machine:Successfully added"));
        } else {
          Setting.showMessage("error", `${i18next.t("machine:Failed to add")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("machine:Failed to add")}: ${error}`);
      });
  }
