...
class ChatWidget extends React.Component {
    ...
    ChatBackend.addChat(newChat)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("general:Failed to create chat")}: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `${i18next.t("general:Failed to create chat")}: ${error}`);
      });
  }
  ...
  sendMessage = (text, fileName, isHidden, isRegenerated) => {
    if (!this.state.currentChat) {
      Setting.showMessage("error", i18next.t("general:No chat available"));
      return;
    }
    ...
    MessageBackend.addMessage(newMessage)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("general:Failed to send message")}: ${res.msg}`);
        }
      })
      .catch(error => {
        this.setState({
    ...
    MessageBackend.getChatMessages("admin", chat.name)
      .then((res) => {
        if (res.status === "ok") {
          ...
          Setting.showMessage("error", `${i18next.t("general:Failed to get messages")}: ${res.msg}`);
          this.setState({
            messageLoading: false,
            messageError: true,
          });
  ...
  renderHeader() {
    ...
    const chatDisplayName = this.state.currentChat?.displayName || title || i18next.t("chat:Chat");
