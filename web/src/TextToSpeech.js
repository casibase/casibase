  ...
  useStreamingTTS(message, storeId, messageId) {
    ...
    this.eventSource.addEventListener("chunk", (e) => {
      try {
        ...
        Setting.showMessage("error", i18next.t("general:TTS stream failed. Falling back to browser TTS."));
        this.useBrowserTTS(message);
      }
    });
    ...
    this.eventSource.addEventListener("error", (e) => {
      try {
        ...
        Setting.showMessage("error", `${i18next.t("general:TTS failed")}: ${errorData.error}. Falling back to browser TTS.`);
      } catch (error) {
        Setting.showMessage("error", i18next.t("general:TTS stream failed. Falling back to browser TTS."));
      }
  ...
  useNonStreamingTTS(message, storeId, messageId) {
    TTSBackend.generateTextToSpeechAudio(storeId, "", messageId, "")
      ...
      .catch(error => {
        ...
        Setting.showMessage("error", `${i18next.t("general:TTS failed")}: ${error.message}. Falling back to browser TTS.`);
        this.useBrowserTTS(message);
      });
  }
