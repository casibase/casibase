...
func updateFileStatus(owner string, storeName string, objectKey string, status FileStatus) error {
	name := getFileName(storeName, objectKey)
	_, err := adapter.engine.ID(core.PK{owner, name}).Cols("status").Update(&File{Status: status})
	return err
}
...
func UpdateFilesStatusByStore(owner string, storeName string, status FileStatus) error {
	_, err := adapter.engine.Where("owner = ? and store = ?", owner, storeName).
		Cols("status").Update(&File{Status: status})
	return err
}
